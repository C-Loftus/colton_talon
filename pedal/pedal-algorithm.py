...
def handle_down_pedal() -> None:
    ...
    if map.multiple_held():
        ...
        elif "east" in pedals and "north" in pedals and "west" in pedals:
...
def handle_held_pedal() -> None:
    ...
    for pedalDirection in map:
        ...
        if held_seconds[pedalDirection] == settings.get(
            "user.secondsToTriggerPedalHold"
        ):
            ...
            if (
                settings.get("user.oneActionOnCenterPress")
                and pedalDirection == "north"
            ):
...
cron.interval(cron.seconds_to_timespec(CHECK_INTERVAL := 0.5), handle_held_pedal)

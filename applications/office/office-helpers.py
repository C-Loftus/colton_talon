...
class Entrance:
    ...
    def check_compiled_file(file_name: str, output_format: str, seconds: int = 5):
        ...
        def kill_program():
            if "powerpoint" in ui.active_app().name.lower() or 'word' in ui.active_app().name.lower() or 'excel' in ui.active_app().name.lower():
                actions.key("alt-f4")
            else:
                print('went to kill application but not in one of the open office applications')

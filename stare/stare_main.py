...
from talon import actions, cron, settings, ui

from .hotspot import getHotspotIfFocused, getHotSpots
...
def sufficient_threshold(focusedLocation) -> bool:
    ...
    if stare_map[focusedLocation] >= to_int(
        settings.get("user.thresholdToTriggerStare")
    ):
        return True
    else:
        print(stare_map)

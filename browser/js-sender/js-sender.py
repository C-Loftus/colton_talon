...
from talon import Context, Module, actions, app, clip, fs
    ...
    if os.path.exists(build_directory) and os.path.isdir(build_directory):
        ...
        for file_name in file_names:
            try:
                ...
                naturalLangName = (
                    naturalLangName.replace("_", " ")
                    if "_" in naturalLangName
                    else naturalLangName
                )
...
mod.list(
    "talon_JS_Functions",
    desc="All the local javascript functions that can be sent to the browser",
)
